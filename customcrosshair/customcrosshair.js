 ...
 * FIXED: data-nx-crosshair tag + safe body attach so loader can inject this file.
 */
(function() {
    if (window.__nxCrosshairActiveStyle === 'original') return;
    window.__nxCrosshairActiveStyle = 'original';

    const oldEl = document.getElementById('unique-crosshair');
    if (oldEl) oldEl.remove();
    document.querySelectorAll('style[data-nx-crosshair]').forEach(s => s.remove());

    const style = document.createElement('style');
    style.setAttribute('data-nx-crosshair', 'original');
    style.innerHTML = `
        ...
        .xhair-core {
            position: absolute; width: 6px; height: 6px; background: #00ffcc;
            transform: rotate(45deg); box-shadow: 0 0 10px #00ffcc, 0 0 20px #00ffcc;
            transition: all 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); z-index: 5;
        }
        .xhair-ring-inner {
            position: absolute; width: 24px; height: 24px; border: 2px dotted rgba(0,255,204,0.6);
            border-radius: 50%; animation: spinR 8s linear infinite;
        }
        .xhair-ring-outer {
            position: absolute; width: 44px; height: 44px; border: 1px dashed rgba(0,255,204,0.4);
            border-radius: 50%; animation: spinN 12s linear infinite;
        }
        ...
        #unique-crosshair.text-input .xhair-core {
            background: #ff00ff; border-radius: 50%; box-shadow: 0 0 15px #ff00ff;
            width: 8px; height: 8px; transform: none;
        }
        #unique-crosshair.text-input .xhair-ring-inner { border-color: rgba(255,0,255,0.5); }
        #unique-crosshair.text-input .xhair-ring-outer { border-color: rgba(255,0,255,0.3); }
        #unique-crosshair.text-input .xhair-bracket { border-color: #ff00ff; }
        #unique-crosshair.text-input .xhair-axis { background: #ff00ff; }
        #unique-crosshair.clicking .xhair-core {
            background: #ff3333; transform: rotate(225deg) scale(2.5);
            box-shadow: 0 0 20px #ff0000, 0 0 40px #ff5555;
        }
        #unique-crosshair.clicking .xhair-bracket { border-color: #ff3333; border-width: 3px; }
        #unique-crosshair.clicking .xhair-axis { background: #ff3333 !important; }
        #unique-crosshair.clicking .xhair-ring-outer { width: 80px; height: 80px; opacity: 0; }
        #unique-crosshair.clicking .xhair-ring-inner { width: 60px; height: 60px; opacity: 0; }
    `;
    ...
    function attachCrosshair() {
        if (!document.body) {
            document.addEventListener('DOMContentLoaded', attachCrosshair, { once: true });
            return;
        }
        if (!document.getElementById('unique-crosshair')) {
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        const isTextInput = (targetTag === 'input' || targetTag === 'textarea' || target.isContentEditable || target.getAttribute('contenteditable') === 'true');
        if (isTextInput) {
            crosshair.classList.add('text-input');
            crosshair.classList.remove('targeting');
        } else {
            crosshair.classList.remove('text-input');
            if (targetTag === 'a' || targetTag === 'button' || targetTag === 'iframe' ||
                target.classList.contains('game-card') || target.closest('.game-card') ||
                target.closest('.game-container') || target.closest('button') || target.closest('a') ||
                target.closest('[role="button"]') || window.getComputedStyle(target).cursor === 'pointer') {
    ...
    document.addEventListener('mouseleave', () => { if (!crosshairEnabled) return; crosshair.classList.add('crosshair-hidden'); });
    document.addEventListener('mouseenter', () => { if (!crosshairEnabled) return; crosshair.classList.remove('crosshair-hidden'); });
