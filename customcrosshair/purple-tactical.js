/**
 * Null_X Crosshair Style — purple-tactical
 */
(function() {
    if (window.__nxCrosshairActiveStyle === 'purple-tactical') return;
    window.__nxCrosshairActiveStyle = 'purple-tactical';
    ...
    style.setAttribute('data-nx-crosshair', 'purple-tactical');
    style.innerHTML = `
        ...
        .xhair-core { position:absolute; width:6px; height:6px; background:#b44dff; transform:rotate(45deg); box-shadow:0 0 10px #8b00ff; z-index:5; transition:all 0.15s; }
        .xhair-axis { position:absolute; background:rgba(180,80,255,0.85); box-shadow:0 0 4px #8b00ff; transition:all 0.12s; }
        .xhair-axis.top { width:2px; height:10px; top:10px; left:calc(50% - 1px); }
        .xhair-axis.bottom { width:2px; height:10px; bottom:10px; left:calc(50% - 1px); }
        .xhair-axis.left { width:10px; height:2px; left:10px; top:calc(50% - 1px); }
        .xhair-axis.right { width:10px; height:2px; right:10px; top:calc(50% - 1px); }
        .xhair-bracket-container { position:absolute; width:56px; height:56px; transition:all 0.18s; }
        .xhair-bracket { position:absolute; width:11px; height:11px; border:2px solid #8b00ff; filter:drop-shadow(0 0 4px #8b00ff); transition:all 0.15s; }
        ...
        #unique-crosshair.targeting .xhair-core { background:#e0a0ff; box-shadow:0 0 14px #b44dff; }
        #unique-crosshair.targeting .xhair-bracket { border-color:#e0a0ff; }
        #unique-crosshair.text-input .xhair-core {
            width:3px; height:18px; transform:none; border-radius:1px; background:#d080ff;
            box-shadow:0 0 12px #8b00ff, 0 0 4px #fff; animation: purplePipe 0.85s ease-in-out infinite;
        }
        #unique-crosshair.text-input .xhair-axis,
        #unique-crosshair.text-input .xhair-bracket-container { opacity:0; transform:scale(0.2); }
        @keyframes purplePipe { 0%,100%{ opacity:1; box-shadow:0 0 12px #8b00ff; } 50%{ opacity:0.3; box-shadow:0 0 4px #8b00ff; } }
        #unique-crosshair.clicking .xhair-core { transform:rotate(225deg) scale(1.8); background:#ff66ff; box-shadow:0 0 22px #ff00ff; }
        #unique-crosshair.clicking .xhair-bracket-container { width:36px; height:36px; }
        #unique-crosshair.clicking .xhair-bracket { border-color:#ff66ff; border-width:3px; }
        #unique-crosshair.clicking .xhair-axis { background:#ff66ff; }
    `;
    document.head.appendChild(style);
    const crosshair = document.createElement('div');
    crosshair.id = 'unique-crosshair';
    crosshair.className = 'crosshair-hidden';
    crosshair.innerHTML = `<div class="xhair-core"></div><div class="xhair-axis top"></div><div class="xhair-axis bottom"></div><div class="xhair-axis left"></div><div class="xhair-axis right"></div><div class="xhair-bracket-container"><div class="xhair-bracket bracket-tl"></div><div class="xhair-bracket bracket-tr"></div><div class="xhair-bracket bracket-bl"></div><div class="xhair-bracket bracket-br"></div></div>`;
